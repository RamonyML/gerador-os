      ...
      ? [
          {
            key: 'dev',
            title: 'Área Dev',
            description: 'Ferramentas internas, integrações e monitoramento. Acesso restrito ao desenvolvedor.',
            to: '/dev',
            icon: <DevIconSvg />,
            accent: '#22c55e',
          } satisfies QuickAction,
          ...
          <Box>
            ...
            {viewMode === 'cards' && (
              ...
              >
                {actions.map((a, index) => {
                  ...
                  const cardAccent = a.accent ?? (isHubCard ? success : primary)
                  return (
                    <Fade
                      key={a.key}
            ...
            {viewMode === 'icons' && (
              ...
              >
                {actions.map((a) => {
                  const isHubCard = a.key === 'suporte' || a.key === 'cadastro' || a.key === 'instalacao'
                  const color = a.accent ?? (isHubCard ? success : primary)
                  return (
                    <Box
                      key={a.key}

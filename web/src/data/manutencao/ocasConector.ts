...
/**
 * Dano ocasionado no conector (interno) — fluxo único com variações de terceiro.
 ...
 *
 * Obs.: o gerador legado imprimia "INFORMAR VALOR DE undefined" na indicação
 * técnica (bug por ${onu.split(' ')[1]} ser sempre indefinido). Aqui o trecho
...
export function buildOcasConectorTextos(
  ...
  if (tipo === T_TITULAR_TERCEIRO) {
    const protocolo = [
      ...
      SEP19,
      sp(20),
      `${cp} CONCORDOU COM A VISITA E CASO HAJA COBRANÇA SOLICITOU PAGAR NO ATO COM ${formaPag}. ${cp} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU ${solicitanteUpper} (${parente}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA ${dataVisita} ÀS ${horaVisita} HRS.`,
      '',
      'CLIENTE SEM DUVIDAS.',
    ].join('\n')
    const osBase = `${cp} ENTROU EM CONTATO POR ${canal} (${contato}) E DISSE QUE ESTÁ SEM CONEXÃO COM A INTERNET. QUESTIONADO DISSE QUE: "${motivo}", E FICOU SEM ACESSO À INTERNET. REMOTAMENTE VERIFIQUEI QUE USUÁRIO ESTÁ DESCONECTADO DO SISTEMA E ${op} APAGADA. EXPLIQUEI QUE COM A QUEDA/INTERVENÇÃO PODE TER DANIFICADO A FIBRA, CONECTOR OU ATÉ MESMO OS EQUIPAMENTOS. INFORMEI A ${cp} QUE É NECESSÁRIO VISITA TÉCNICA PARA REPARO, SENDO PROBLEMA OCASIONADO (ESPONTANEO OU NÃO) COBRA-SE VISITA TÉCNICA DE R$50,00 E ATÉ MESMO EQUIPAMENTOS SE DANIFICADOS. ${cp} CONCORDOU COM A VISITA E CASO HAJA COBRANÇA SOLICITOU PAGAR NO ATO COM ${formaPag}. ${cp} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU ${solicitanteUpper} (${parente}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA ${dataVisita} ÀS ${horaVisita} HRS.`
    return {
      ocasConectTextoProtocolo: protocolo,
  ...
  const osBase = `${cp} ENTROU EM CONTATO POR ${canal} (${contato}) E DISSE QUE ESTÁ SEM CONEXÃO COM A INTERNET. QUESTIONADO DISSE QUE: "${motivo}", E FICOU SEM ACESSO À INTERNET. REMOTAMENTE VERIFIQUEI QUE USUÁRIO ESTÁ DESCONECTADO DO SISTEMA E ${op} APAGADA. EXPLIQUEI QUE COM A QUEDA/INTERVENÇÃO PODE TER DANIFICADO A FIBRA, CONECTOR OU ATÉ MESMO OS EQUIPAMENTOS. INFORMEI A ${cp} QUE É NECESSÁRIO VISITA TÉCNICA PARA REPARO, SENDO PROBLEMA OCASIONADO (ESPONTANEO OU NÃO) COBRA-SE VISITA TÉCNICA DE R$50,00 E ATÉ MESMO EQUIPAMENTOS SE DANIFICADOS. ${cp} CONCORDOU COM OS TERMOS DA VISITA E PAGARÁ EM ${formaPag}. VISITA AGENDADA PARA ${dataVisita} À PARTIR DE ${horaVisita} HRS.`
  return {
    ocasConectTextoProtocolo: protocolo,
    ocasConectTextoOS: osBase + ctoLine + osTailBlank(op),
    ocasConectTextoAgenda: agenda,
...
export const OCAS_CONECTOR_FIELDS: OsTemplateField[] = [
  ...
  },
  {
    ...
    control: 'text',
    placeholder: "Ex.: 'ao mover o sofá de lugar, esbarrou na fibra e danificou o conector'",
    section: S_DET,
    layout: { md: 12 },
  },
  {

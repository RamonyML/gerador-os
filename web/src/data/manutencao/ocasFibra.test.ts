...
const tecnico = (quem: string) =>
  `TECNICO: VERIFICAR DROP INTERNO E EXTERNO, SE SOBRA TECNICA FOR SUFICIENTE, USAR PARA REPARO E RESTABELECER CONEXAO. CASO NAO SEJA PASSAR OUTRO DROP. CORRIGIR QUALQUER INCONSISTENCIAS NA INSTALACAO QUE NAO TIVER PADRAO. AO FINALIZAR ENTRAR EM CONTATO COM SUPORTE PARA CONFERIR SINAL E CONFIRMAR NORMALIZACAO COM ${quem}. TEMPO ESTIMADO 60 MIN.`
...
function legadoPadrao(v: Vars, c: 'CTOE' | 'CTOI') {
  ...
  const protocolo = [
    ...
    '',
    `QUESTIONADO, DISSE QUE A ${op} ESTA COM LUZ VERMELHA ACESA. PERGUNTEI O MOTIVO E ${cp} DISSE QUE "${v.motivo}".`,
    '',
    v.valor,
    '',
    SEP42,
  ...
  const osBase = `${cp} ENTROU EM CONTATO POR ${v.canal} (${v.contato}) E DISSE QUE ESTA SEM CONEXAO COM A INTERNET. QUESTIONADO DISSE QUE: "${v.motivo}", E FICOU SEM ACESSO A INTERNET. PERGUNTEI SOBRE A ${op}, E CLIENTE DISSE QUE ESTA COM LUZ VERMELHA ACESA. REMOTAMENTE VERIFIQUEI QUE USUARIO ESTA DESCONECTADO E ONU APAGADA. ${v.valor} ${cp} AUTORIZOU VISITA E PAGARA EM ${v.formaPag} NO ATO. VISITA AGENDADA PARA ${v.dataVisita} AS ${v.horaVisita} HRS.`
...
function legadoFibra3(v: Vars, c: 'CTOE' | 'CTOI') {
  ...
  const protocolo = [
    ...
    SEP19,
    sp(20),
    `${cp} CONCORDOU COM A VISITA E FARA O PAGAMENTO COM ${v.formaPag}. ${cp} DISSE QUE NAO ESTARA PRESENTE, MAS AUTORIZOU ${v.solicitante} (${v.parente}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA ${v.dataVisita} AS ${v.horaVisita} HRS.`,
    '',
    'CLIENTE SEM DUVIDAS.',
  ].join('\n')
  const osBase = `${cp} ENTROU EM CONTATO POR ${v.canal} (${v.contato}) E DISSE QUE ESTA SEM CONEXAO COM A INTERNET. QUESTIONADO DISSE QUE: "${v.motivo}", E FICOU SEM ACESSO A INTERNET. REMOTAMENTE VERIFIQUEI QUE USUARIO ESTA DESCONECTADO DO SISTEMA E ${op} APAGADA. ${v.valor} ${cp} CONCORDOU COM A VISITA E FARA O PAGAMENTO COM ${v.formaPag}. ${cp} DISSE QUE NAO ESTARA PRESENTE, MAS AUTORIZOU ${v.solicitante} (${v.parente}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA ${v.dataVisita} AS ${v.horaVisita} HRS.`
  const os = osBase + ctoBlock(c, v.cto, v.passante) + `${SEP_OS}\n\nINDICACAO TECNICA:\n\n${tecnico(cp)}`
  return { protocolo, os, agenda: agenda(v, c) }
}
...
const BASE: Vars = {
  ...
  onu: 'ONU',
  bairro: 'SARAIVA',
  motivo: 'AO PODAR UMA ARVORE, CLIENTE CORTOU A FIBRA OTICA EXTERNA',
  valor: VALOR,

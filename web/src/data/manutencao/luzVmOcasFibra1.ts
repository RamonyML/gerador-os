...
QUESTIONADO, {{solicitantePrimeiro}} DISSE QUE A {{onuPrimeiro}} ESTÁ COM LUZ VERMELHA ACESA. PERGUNTEI O MOTIVO E {{solicitantePrimeiro}} DISSE QUE "{{motivoUpper}}", E FICOU SEM ACESSO À INTERNET.
...
POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR {{canal}} ({{contatoNumerico}}) COM {{clientePrimeiro}} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU {{solicitanteUpper}} ({{parenteUpper}}) ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. {{clientePrimeiro}} CONCORDOU COM OS TERMOS DA VISITA TÉCNICA E FARÁ O PAGAMENTO EM {{formaPag}}. VISITA AGENDADA PARA O DIA {{dataVisita}} ÀS {{horaVisita}} HRS.

CLIENTE SEM DUVIDAS.

=== Texto O.S ===
{{solicitantePrimeiro}} ({{parenteUpper}} DE {{clientePrimeiro}}) ENTROU EM CONTATO POR {{canal}} ({{contatoSolNumerico}}) E DISSE QUE ESTÁ SEM CONEXÃO COM A INTERNET. QUESTIONADO DISSE QUE: "{{motivoUpper}}", E FICOU SEM ACESSO À INTERNET. REMOTAMENTE VERIFIQUEI QUE USUÁRIO ESTÁ DESCONECTADO DO SISTEMA E {{onuPrimeiro}} APAGADA. EXPLIQUEI QUE COM A QUEDA/INTERVENÇÃO PODE TER DANIFICADO A FIBRA, CONECTOR OU ATÉ MESMO OS EQUIPAMENTOS. {{valor}} {{solicitantePrimeiro}} CONCORDOU COM A VISITA E CASO HAJA COBRANÇA SOLICITOU PAGAR NO ATO COM {{formaPag}}. POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR {{canal}} ({{contatoNumerico}}) COM {{clientePrimeiro}} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU {{solicitanteUpper}} ({{parenteUpper}}) ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA {{dataVisita}} ÀS {{horaVisita}} HRS.{{textoOSCtoSinalAlto}}
******************************************

INDICAÇÃO TÉCNICA:

TÉCNICO: VERIFICAR DROP INTERNO E EXTERNO, SE SOBRA TÉCNICA FOR SUFICIENTE, USAR PARA REPARO E RESTABELECER CONEXÃO. CASO NÃO SEJA PASSAR OUTRO DROP. CORRIGIR QUALQUER INCONSISTÊNCIAS NA INSTALAÇÃO QUE NÃO TIVER PADRÃO. AO FINALIZAR ENTRAR EM CONTATO COM SUPORTE PARA CONFERIR SINAL E CONFIRMAR NORMALIZAÇÃO COM {{solicitantePrimeiro}}. TEMPO ESTIMADO 60 MIN.
...
export function getManutLuzVmOcasFibra1Defaults(): OsTemplatePresetPayload {
  return {
    slug: 'manut-luz-vm-ocas-fibra-1',
    title: 'Luz vermelha ocasionado (fibra) — terceiro (fluxo 1)',

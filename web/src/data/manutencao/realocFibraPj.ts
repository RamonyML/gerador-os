...
const S_ID = 'IDENTIFICAÇÃO (PJ)'
const S_SOL = 'DETALHES DA SOLICITAÇÃO'
const S_AGE = 'AGENDAMENTO'

const OUTPUT = `=== Texto Protocolo ===
{{solicitantePrimeiro}} ({{cargoUpper}}) ENTROU EM CONTATO POR {{canal}} ({{contatoNumerico}}) E SOLICITOU SUPORTE.
...
=== Texto O.S ===
{{solicitantePrimeiro}} ({{cargoUpper}}) ENTROU EM CONTATO POR {{canal}} ({{contatoNumerico}}) PARA SOLICITAR SUPORTE. QUESTIONADO, DISSE "{{motivoUpper}}". {{valor}} CLIENTE PAGARÁ EM {{formaPag}}. AGENDADA PARA {{dataVisita}} ÀS {{horaVisita}} HORAS.
...
const FIELDS: OsTemplateField[] = [
  ...
  {
    id: 'solicitante',
    label: 'Representante / solicitante',
    control: 'text',
    section: S_ID,
    layout: { md: 4 },
  },
...
export function getManutRealocFibraPjDefaults(): OsTemplatePresetPayload {
  return {
    slug: 'manut-realoc-fibra-pj',
    title: 'Remanejamento de fibra — PJ',

...
/** Legado mudponto2.html — terceiro contacta; assinante autoriza terceiro; ONU com SEM OSCILAÇÃO. */
const OUTPUT = `=== Texto Protocolo ===
{{solicitantePrimeiro}} ({{parenteUpper}} DE {{clientePrimeiro}}) ENTROU EM CONTATO POR {{canal}} ({{contatoSolNumerico}}) SOLICITANDO INFORMAÇÕES SOBRE MUDANÇA DE PONTO INTERNO
...
POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR {{canal}} ({{contatoNumerico}}) COM {{clientePrimeiro}} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU {{solicitanteUpper}} ({{parenteUpper}}) ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO CASO HOUVER. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA {{dataVisita}} ÀS {{horaVisita}} HRS.

=== Texto O.S ===
{{solicitantePrimeiro}} ({{parenteUpper}} DE {{clientePrimeiro}}) ENTROU EM CONTATO POR {{canal}} ({{contatoSolNumerico}}) MUDANÇA DE PONTO INTERNO, RETIRAR EQUIPAMENTOS DE: {{ambienteAtualUpper}}, E REINSTALAR EM: {{ambienteNovoUpper}}. MOTIVO: {{motivoUpper}}. {{valor}} {{solicitantePrimeiro}} SOLICITOU PAGAR EM {{formaPag}}. POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR {{canal}} ({{contatoNumerico}}) COM {{clientePrimeiro}} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU {{solicitanteUpper}} ({{parenteUpper}}) ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO CASO HOUVER. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA {{dataVisita}} ÀS {{horaVisita}} HRS.
...
const FIELDS: OsTemplateField[] = [
  ...
  {
    id: 'solicitante',
    label: 'Quem entrou em contato (nome)',
    control: 'text',
    section: S_ID,
    layout: { md: 6 },
  },
...
export function getManutMudPontoInternoTerceiroAutorizaDefaults(): OsTemplatePresetPayload {
  return {
    slug: 'manut-mud-ponto-int-terceiro-autoriza',
    title: 'Mudança de ponto interno — terceiro (assinante autoriza)',

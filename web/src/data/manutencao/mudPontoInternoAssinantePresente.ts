...
/** Legado mudponto3.html — terceiro contacta; assinante confirma presença e pagamento. */
...
POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR {{canal}} ({{contatoNumerico}}) COM {{clientePrimeiro}} (ASSINANTE) QUE CONFIRMOU E DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA {{dataVisita}} ÀS {{horaVisita}} HRS.

=== Texto O.S ===
{{solicitantePrimeiro}} ({{parenteUpper}} DE {{clientePrimeiro}}) ENTROU EM CONTATO POR {{canal}} ({{contatoSolNumerico}}) MUDANÇA DE PONTO INTERNO, RETIRAR EQUIPAMENTOS DE: {{ambienteAtualUpper}}, E REINSTALAR EM: {{ambienteNovoUpper}}. MOTIVO: {{motivoUpper}}. {{valor}} {{solicitantePrimeiro}} ESCOLHEU PAGAR EM {{formaPag}}. POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR {{canal}} ({{contatoNumerico}}) COM {{clientePrimeiro}} (ASSINANTE) QUE CONFIRMOU E DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO CASO HOUVER. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA {{dataVisita}} ÀS {{horaVisita}} HRS.
...
export function getManutMudPontoInternoAssinantePresenteDefaults(): OsTemplatePresetPayload {
  return {
    slug: 'manut-mud-ponto-int-assinante-presente',
    title: 'Mudança de ponto interno — terceiro (assinante presente)',

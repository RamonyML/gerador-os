...
{{solicitantePrimeiro}} ({{cargoUpper}}) ENTROU EM CONTATO POR {{canal}} ({{contatoNumerico}}) SOLICITANDO INFORMAÇÕES SOBRE MUDANÇA DE PONTO INTERNO
---
...
{{solicitantePrimeiro}} CONCORDOU COM OS TERMOS DA VISITA TÉCNICA, PAGAMENTO SERÁ FEITO NO ATO EM {{formaPag}}, DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR O TÉCNICO. VISITA AGENDADA PARA O DIA {{dataVisita}} ÀS {{horaVisita}} HRS.

=== Texto O.S ===
{{solicitantePrimeiro}} ({{cargoUpper}}) SOLICITOU POR {{canal}} ({{contatoNumerico}}) MUDANÇA DE PONTO INTERNO, RETIRAR EQUIPAMENTOS DE: {{ambienteAtualUpper}}, E REINSTALAR EM: {{ambienteNovoUpper}}. MOTIVO: {{motivoUpper}}. {{valor}} CLIENTE PAGARÁ EM {{formaPag}}. AGENDADA PARA {{dataVisita}} ÀS {{horaVisita}} HORAS.
...
const FIELDS: OsTemplateField[] = [
  {
    id: 'cliente',
    label: 'Razão social / nome do contrato',
    control: 'text',
    section: S_ID,
    layout: { md: 6 },
  },
...
export function getManutMudPontoInternoPjDefaults(): OsTemplatePresetPayload {
  return {
    slug: 'manut-mud-ponto-int-pj',
    title: 'Mudança de ponto interno — PJ',

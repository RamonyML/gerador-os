...
/** Legado mudponto1.html — titular ausente; autorizado acompanha. */
...
{{clientePrimeiro}} CONCORDOU COM OS TERMOS DA VISITA TÉCNICA, PAGAMENTO SERÁ FEITO NO ATO EM {{formaPag}}, {{clientePrimeiro}} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU {{autorizadoUpper}} ({{parenteUpper}}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO CASO HOUVER. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA {{dataVisita}} ÀS {{horaVisita}} HRS.

=== Texto O.S ===
{{clientePrimeiro}} SOLICITOU POR {{canal}} ({{contatoNumerico}}) MUDANÇA DE PONTO INTERNO, RETIRAR EQUIPAMENTOS DE: {{ambienteAtualUpper}}, E REINSTALAR EM: {{ambienteNovoUpper}}. MOTIVO: {{motivoUpper}}. {{valor}} CLIENTE PAGARÁ EM {{formaPag}}. {{clientePrimeiro}} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU {{autorizadoUpper}} ({{parenteUpper}}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO CASO HOUVER. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA {{dataVisita}} ÀS {{horaVisita}} HRS.
...
export function getManutMudPontoInternoAutorizadoDefaults(): OsTemplatePresetPayload {
  return {
    slug: 'manut-mud-ponto-int-autorizado',
    title: 'Mudança de ponto interno — titular ausente (autorizado)',

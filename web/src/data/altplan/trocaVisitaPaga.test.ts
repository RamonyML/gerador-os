...
`
  ...
  const agenda = `ALT PLANO ${cliente} PROT:${v.protocolo} ${formaPag} (${v.operadorPrimeiroNome}) - ${bairro} // ${roteadorSug}`

  if (tipo === TVP_TITULAR_TERCEIRO) {
    const protocolo = `${base(introTitular)}
${clientePrimeiro} ESTÁ CIENTE DA RENOVAÇÃO DA FIDELIDADE POR 12 MESES E CONCORDOU COM OS TERMOS. OPTOU POR REALIZAR O PAGAMENTO NO ATO EM ${formaPag}. ${clientePrimeiro} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU ${autorizado} (${parente}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA ${v.dataVisita} ÀS ${v.horaVisita} HRS.

CLIENTE SEM DUVIDAS.`
    const os = osComIndicacao(
      `${osIntroTitular} ${clientePrimeiro} CONCORDOU COM A VISITA, DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU ${autorizado} (${parente}) A ACOMPANHAR O TÉCNICO E ASSINAR O.S. ${visitaPagaOS} VISITA AGENDADA PARA ${v.dataVisita} ÀS ${v.horaVisita} HRS.`,
    )
...
CLIENTE SEM DUVIDAS.`
    const os = osComIndicacao(
      `${osIntroTerceiro} POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR ${canal} (${contato}) COM ${clientePrimeiro} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU A VISITA. DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR O TÉCNICO E ASSINAR O.S. ${visitaPagaOS} AGENDADA PARA O DIA ${v.dataVisita} ÀS ${v.horaVisita} HRS.`,
    )
    return { protocolo, os, agenda }
  }
  ...
  if (tipo === TVP_TERCEIRO_TERCEIRO) {
    const protocolo = `${base(introTerceiro)}
POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR ${canal} (${contato}) COM ${clientePrimeiro} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU ${solicitante} (${parente}) ACOMPANHAR O TÉCNICO E ASSINAR O.S. ${clientePrimeiro} CONCORDOU COM OS TERMOS DE ALTERAÇÃO DE PLANO, E VALIDOU SOBRE A RENOVAÇÃO DA FIDELIDADE POR 12 MESES. PAGAMENTO SERÁ REALIZADO EM ${formaPag} VISITA AGENDADA PARA O DIA ${v.dataVisita} ÀS ${v.horaVisita} HRS.

CLIENTE SEM DUVIDAS.`
    const os = osComIndicacao(
      `${osIntroTerceiro} POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR ${canal} (${contato}) COM ${clientePrimeiro} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU ${solicitante} (${parente}) ACOMPANHAR O TÉCNICO E ASSINAR O.S. ${visitaPagaOS} AGENDADA PARA ${v.dataVisita} ÀS ${v.horaVisita} HRS.`,
    )
    return { protocolo, os, agenda }
  }

  const protocolo = `${base(introTitular)}
${clientePrimeiro} ESTÁ CIENTE DA RENOVAÇÃO DA FIDELIDADE POR 12 MESES E CONCORDOU COM OS TERMOS. OPTOU POR REALIZAR O PAGAMENTO NO ATO EM ${formaPag}, E A VISITA TÉCNICA FOI AGENDADA PARA O DIA ${v.dataVisita} ÀS ${v.horaVisita} HRS, DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR O TÉCNICO.`
  const os = osComIndicacao(
    `${osIntroTitular} ${visitaPagaOS} VISITA AGENDADA PARA ${v.dataVisita} ÀS ${v.horaVisita} HRS.`,
  )
  return { protocolo, os, agenda }
}
...
function gerarNovo(v: Entrada) {
  const context = {
    ...v,
    ...buildAltplanTrocaVisitaPagaTextos(v, v.operadorPrimeiroNome),
  }
  const sections = splitOsPreviewSections(
    renderTemplate(ALTPLAN_TROCA_VISITA_PAGA_OUTPUT, context),
  )
  return {
...
const VARIACOES: Array<{ nome: string; tipo: string }> = [
  { nome: 'Titular solicita e acompanha', tipo: TVP_TITULAR },
  { nome: 'Titular solicita e autoriza terceiro', tipo: TVP_TITULAR_TERCEIRO },
  { nome: 'Terceiro solicita e titular acompanha', tipo: TVP_TERCEIRO_TITULAR },
  {
    nome: 'Terceiro solicita e terceiro acompanha',
    tipo: TVP_TERCEIRO_TERCEIRO,
  },
]

describe('Alteração de plano — com troca visita paga', () => {
...
describe('Alteração de plano — com troca visita paga (ofertado)', () => {
  for (const tipo of [TVP_TITULAR, TVP_TERCEIRO_TERCEIRO]) {

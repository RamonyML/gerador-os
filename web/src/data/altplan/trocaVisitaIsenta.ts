 ...
 * - legado-exemplo/suporte/altplan/altplan-troca-visita-isenta/index-altplan-troca-visita-isenta.html
 *
 * Variações de terceiro consolidadas a partir de:
 * - ct-vi-1 (titular solicita / autoriza terceiro)
 * - ct-vi-3 (terceiro solicita / titular acompanha)
 * - ct-vi-2 (terceiro solicita / terceiro acompanha)
 * Separadores normalizados para o padrão de asteriscos da versão titular.
 */
...
export const TVI_TITULAR = 'titular-solicita-titular-acompanha'
export const TVI_TITULAR_TERCEIRO = 'titular-solicita-terceiro-acompanha'
export const TVI_TERCEIRO_TITULAR = 'terceiro-solicita-titular-acompanha'
export const TVI_TERCEIRO_TERCEIRO = 'terceiro-solicita-terceiro-acompanha'
...
export const ALTPLAN_TROCA_VISITA_ISENTA_OUTPUT = [
  '=== Texto Protocolo ===',
  '{{altplanTrocaVisitaIsentaTextoProtocolo}}',
  '',
  '=== Texto O.S ===',
  '{{altplanTrocaVisitaIsentaTextoOS}}',
  '',
  '=== Texto da Agenda ===',
  '{{altplanTrocaVisitaIsentaTextoAgenda}}',
].join('\n')
...
export function buildAltplanTrocaVisitaIsentaTextos(
  ...
  const textoProtocolo = [
    ...protocoloBase(introTitular, sig, compat, valores),
    `${clientePrimeiro} ESTÁ CIENTE DA RENOVAÇÃO DA FIDELIDADE POR 12 MESES E CONCORDOU COM OS TERMOS, E VISITA TÉCNICA ISENTA DE CUSTOS FOI AGENDADA PARA O DIA ${dataVisita} ÀS ${horaVisita} HRS, DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR O TÉCNICO.`,
  ].join('\n')
...
export function buildAltplanTrocaVisitaIsentaSegmentos(
  rawValues: Record<string, unknown>,
  ...
  const isThird          = tipo === TVI_TERCEIRO_TITULAR || tipo === TVI_TERCEIRO_TERCEIRO
  ...
  const informei2 = isSim
    ? `APÓS INSTALADO, FAREMOS OS TESTES DE ABRANGÊNCIA, QUALIDADE, VELOCIDADE E SANAR TODAS AS DÚVIDAS QUE CLIENTE/USUÁRIOS POSSAM TER. VISITA ISENTA DE CUSTOS.`
    : `REALIZAR OS TESTES DE ABRANGÊNCIA, QUALIDADE, VELOCIDADE E SANAR TODAS AS DÚVIDAS QUE CLIENTE/USUÁRIOS POSSAM TER. VISITA ISENTA DE CUSTOS.`
  ...
  if (tipo === TVI_TITULAR_TERCEIRO) {
  ...
  } else if (tipo === TVI_TERCEIRO_TITULAR) {
  ...
  } else if (tipo === TVI_TERCEIRO_TERCEIRO) {
    ...
    fechamento = `${cp} ESTÁ CIENTE DA RENOVAÇÃO DA FIDELIDADE POR 12 MESES E CONCORDOU COM OS TERMOS, E VISITA TÉCNICA ISENTA DE CUSTOS FOI AGENDADA PARA O DIA ${dataVisita} ÀS ${horaVisita} HRS, DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR O TÉCNICO.`
  }
  ...
  const { altplanTrocaVisitaIsentaTextoOS } = buildAltplanTrocaVisitaIsentaTextos(rawValues, '')
  const sep = `\n\n${'*'.repeat(35)}\n\nINDICAÇÃO TÉCNICA:\n\n`
  const splitAt = altplanTrocaVisitaIsentaTextoOS.indexOf(sep)
  const osDescricao = splitAt >= 0 ? altplanTrocaVisitaIsentaTextoOS.slice(0, splitAt) : altplanTrocaVisitaIsentaTextoOS
  const osIndicacoes = splitAt >= 0 ? altplanTrocaVisitaIsentaTextoOS.slice(splitAt + sep.length) : ''
...
const THIRD_REQUEST = [TVI_TERCEIRO_TITULAR, TVI_TERCEIRO_TERCEIRO]

export const ALTPLAN_TROCA_VISITA_ISENTA_FIELDS: OsTemplateField[] = [
  {
    id: 'tipoSolicitacao',
    label: 'Tipo de solicitação',
    control: 'select',
    highlight: true,
    defaultValue: TVI_TITULAR,
    options: [
      {
        value: TVI_TITULAR,
  ...
  },
  {
    ...
    section: S_TERCEIRO,
    showWhen: { field: 'tipoSolicitacao', equals: TVI_TITULAR_TERCEIRO },
    layout: { md: 5 },
  },
  {
    id: 'parente',
...
export function getAltplanTrocaVisitaIsentaDefaults() {
  return {
    slug: 'altplan-troca-visita-isenta',
    title: 'Alteração de plano — com troca visita isenta',
    outputTemplate: ALTPLAN_TROCA_VISITA_ISENTA_OUTPUT,
    demandCategory: 'alteracao-plano',
    fields: ALTPLAN_TROCA_VISITA_ISENTA_FIELDS.map((f) => ({
      ...f,

...
function gerarTextos() {
    ...
    const solicitante = document.getElementById('solicitante').value.toUpperCase();
    ...
    const textoProtocolo = `${solicitante.split(' ')[0]} (${cargo}) ENTROU EM CONTATO POR ${canal} (${contato}) INFORMANDO PROBLEMA DE CONEXÃO.
...
QUESTIONADO, DISSE QUE A ${onu.split(' ')[0]} ESTÁ COM LUZ VERMELHA ACESA.
...
${solicitante.split(' ')[0]} CONCORDOU COM OS TERMOS DA VISITA TÉCNICA E CASO HAJA CUSTOS PAGARÁ EM ${formaPag}, DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR O TÉCNICO. VISITA AGENDADA PARA O DIA ${dataVisita} ÀS ${horaVisita} HRS.

CLIENTE SEM DUVIDAS.`;


const textoOS = `${solicitante.split(' ')[0]} (${cargo}) ENTROU EM CONTATO POR ${canal} (${contato}) E DISSE QUE ESTÁ SEM CONEXÃO COM A INTERNET. QUESTIONADO DISSE QUE: ${motivo}, E FICOU SEM ACESSO À INTERNET. PERGUNTEI SOBRE A ${onu.split(' ')[0]}, E CLIENTE DISSE QUE ESTÁ COM LUZ VERMELHA ACESA. REMOTAMENTE VERIFIQUEI QUE USUÁRIO ESTÁ DESCONECTADO E ${onu.split(' ')[0]} APAGADA. EXPLIQUEI QUE É NECESSÁRIO REPARO DO DROP, TÉCNICO CONSEGUINDO REAPROVEITAR O CABO, OU CASO NÃO SEJA POSSÍVEL REAPROVEITÁ-LO SENDO NECESSÁRIO FAZER EMENDA TÉCNICA, O VALOR É DE R$ 50,00 REFERENTE A MÃO DE OBRA TÉCNICA. ${solicitante.split(' ')[0]} AUTORIZOU VISITA E PAGARÁ EM ${formaPag} NO ATO. VISITA AGENDADA PARA ${dataVisita} ÀS ${horaVisita} HRS.

******************************************

INDICAÇÃO TÉCNICA:

TÉCNICO: VERIFICAR POSSIBILIDADE REAPROVEITAR CABO DROP USANDO A SOBRA E RECONECTORIZAR, SE NÃO DER TAMANHO, VERIFICAR POSSIBILIDADE DE INCLUIR OUTRA PARTE DE DROP COM EMENDA E COBRAR A VISITA MÍNIMA DE R$50,00. CONFERIR INSTALAÇÃO DOS EQUIPAMENTOS SE TIVER ALGO FORA DO PADRÃO, APRESENTAR E VERIFICAR SE CLIENTE ACEITA CORRIGIR. ATUALIZAR FIRMWARE DO ROTEADOR SE NECESSÁRIO. VERIFICAR COM CLIENTE (OU COM QUEM ACOMPANHAR) SE HÁ DÚVIDAS A SER ESCLARECIDAS. <b> CTO: ${cto} // ${passante}.</b>`;

...
function gerarTextos() {
    ...
    const textoProtocolo = `POR ${canal} (${contato}) OFERTEI À ${cliente.split(' ')[0]} ALTERAÇÃO DE PLANO COM WI-FI EXTEND.
...
const textoOS = `POR ${canal} (${contato}) OFERTEI À ${cliente.split(' ')[0]} ALTERAÇÃO DE PLANO DE INTERNET: PLANO ATUAL: ${planoAtual}. PLANO ESCOLHIDO: ${planoEscolhido}; VENCIMENTO: DIA ${vencimentoData} DO MÊS; VIGÊNCIA DO CONTRATO: 12 MESES (VIDE CONTRATO). VISITA AGENDADA PARA ${dataVisita} A PARTIR DE ${horaVisita} HRS.

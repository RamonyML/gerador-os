...
function gerarTextos() {
    ...
    const textoProtocolo = `${solicitante.split(' ')[0]} (${cargo}) ENTROU EM CONTATO VIA ${canal} (${contato}) SOLICITANDO INFORMAÇÕES SOBRE WI-FI EXTEND.
...
QUESTIONADO, ${solicitante.split(' ')[0]} INFORMOU QUE SUA RESIDÊNCIA É GRANDE E A REDE WI-FI NÃO ABRANGE TODA A ÁREA DE SUA RESIDENCIA.
...
${solicitante.split(' ')[0]} ESTÁ CIENTE DA RENOVAÇÃO DA FIDELIDADE POR 12 MESES E CONCORDOU COM OS TERMOS, E VISITA TÉCNICA ISENTA DE CUSTOS FOI AGENDADA PARA O DIA ${dataVisita} A PARTIR DE ${horaVisita} HRS, DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR O TÉCNICO.`;


const textoOS = `${solicitante.split(' ')[0]} (${cargo}) SOLICITOU POR ${canal} (${contato}) ALTERAÇÃO DO PLANO DE INTERNET: PLANO ATUAL: ${planoAtual}. PLANO ESCOLHIDO: ${planoEscolhido}; VENCIMENTO: DIA ${vencimentoData} DO MÊS; VIGÊNCIA DO CONTRATO: 12 MESES (VIDE CONTRATO). VISITA AGENDADA PARA ${dataVisita} A PARTIR DE ${horaVisita} HRS.
